...
with open('model/anime-dataset.pkl', 'rb') as file:
...
def case_based_filtering_by_genre(anime_name, n=10):
    
    anime_row = df_anime[df_anime['Name'] == anime_name].iloc[0]
    genres = set(anime_row['Genres'].split(', '))

    similarities = []
    for idx, row in df_anime.iterrows():
        other_genres = set(row['Genres'].split(', '))
        jaccard_similarity = len(genres & other_genres) / len(genres | other_genres)
    ...
    for similarity in similarities:
        ...
        similarity_percentage = f"{anime['similarity'] * 100:.2f}%"
        
        anime_details = df_anime[df_anime['anime_id'] == anime_id].iloc[0]
        ...
        recommended_animes.append({
            ...
            "Rank": anime_details['Rank']
        })
    
    return pd.DataFrame(recommended_animes)

def content_rating_similarity(rating1, rating2):
    """
    Compares two content ratings and returns a similarity score.
    """
    # Define an order for the content ratings
    rating_order = ['G', 'PG', 'PG-13', 'R - 17+', 'NC-17']
    
    # Extract the main rating category (e.g., 'R' from 'R - 17+')
    rating1_category = rating1.split()[0]  # e.g., "R"
    rating2_category = rating2.split()[0]  # e.g., "PG"
    
    # Calculate similarity based on their positions in the rating_order list
    similarity = 1 - abs(rating_order.index(rating1_category) - rating_order.index(rating2_category)) / len(rating_order)
    return similarity

# Content Rating-based Filtering Function
def case_based_filtering_by_content_rating(anime_name, n=10):
    
    anime_row = df_anime[df_anime['Name'] == anime_name].iloc[0]
    content_rating = anime_row['Rating']  # Get the rating of the input anime
    
    # Filter other anime with the same content rating
    recommended_animes = df_anime[df_anime['Rating'] == content_rating].head(n)
    
    # Prepare and return the recommendations
    return recommended_animes[['Name', 'Rating', 'Genres', 'Image URL', 'Score']]

def case_based_filtering_by_score(anime_name, n=10):
    anime_row = df_anime[df_anime['Name'] == anime_name].iloc[0]
    score = anime_row['Score']

    # Calculate similarity based on the difference in scores
    similarities = []
    for idx, row in df_anime.iterrows():
        other_score = row['Score']
        score_similarity = 1 - abs(float(score) - float(other_score)) / 10  # Normalize similarity (assuming score scale of 1-10)
        
        if score_similarity > 0:  # Only consider positive similarities
            similarities.append({
                "anime_name": row['Name'],
...
def recommend():
    ...
    elif recommendation_type == "case_based":
        anime_name = request.form['anime_name']
        # Based on case_id, determine the filtering logic
        if case_id == 'Genres':
            recommended_animes = case_based_filtering_by_genre(anime_name, n=num_recommendations)
        elif case_id == 'Rating':
            recommended_animes = case_based_filtering_by_content_rating(anime_name, n=num_recommendations)
        elif case_id == 'Score':
            recommended_animes = case_based_filtering_by_score(anime_name, n=num_recommendations)
        else:
            return render_template('index.html', error_message="Invalid case selection.", recommendation_type=recommendation_type)
        return render_template('recommendations.html', animes=recommended_animes, recommendation_type=recommendation_type)
